...
app.get("/api/auth/google/callback", async (req, res) => {
  ...
  if (error) {
    ...
    req.session.message = "Authentication failed: " + error;
    return res.redirect('/');
  }

  if (!code) {
    console.error('No code received');
    req.session.message = "Authentication failed: No authorization code received";
    return res.redirect('/');
  }
  ...
  try {
    ...
    if (!user) {
      ...
      if (refresh_token || data.id_token) {
        user.refresh_token = refresh_token || data.id_token;
      }
      user.expires_at_ts = expiresAtTs;
      user.expires_at = expiresAt;
      user.scopes = scope ? scope.split(" ") : user.scopes || [];
      await user.save();
      console.log('Existing user updated:', profile.email);
    }
